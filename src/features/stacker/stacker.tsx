...
export default function Stacker({ selectedVideo, onClear }: StackerProps) {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const [stackResult] = await Promise.all([
          stackVideo(selectedVideo, DEFAULT_STACK_SETTINGS, controller.signal),
          minDuration,
        ]);
        if (ignore) return;
  ...
  return <ProcessingPhase fileName={selectedVideo.name} onCancel={onClear} />;
}
